...
constexpr inline auto const true_proportional_navigation_command(
...
)
{
    ...
    auto const& Vtm{ target_velocity - interceptor_velocity };
    return 
        (
            effective_navigation_ratio
            *
                ( line_of_sight | Vtm )
            *   
                ( line_of_sight ^ Vtm )
            /
                math::squared( line_of_sight_squared_length )
        )
    *
        math::real2 < RealType > { -line_of_sight.y, line_of_sight.x};
};
...
int main() {
    ...
    // Вызываем функцию true_proportional_navigation_command
    auto result = true_proportional_navigation_command(line_of_sight, target_velocity, interceptor_velocity, effective_navigation_ratio);
